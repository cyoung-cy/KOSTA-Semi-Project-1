...
public class MovieListCollector {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            while (savedCount < 120 && !remainGenreCount.isEmpty()) {
                ...
                List<MovieAPI> dailyMovies = KobisDailyBoxOfficeAPI.getDailyTop10(targetDt);

                if (dailyMovies.isEmpty()) {
                    System.out.println("조회 결과 없음");
                    System.out.println("현재 조회일 기준 한 달 전으로 이동합니다.");
                    searchDate = searchDate.minusMonths(1);
                    Thread.sleep(100);
                    continue;
                }

                for (MovieAPI boxOfficeMovie : dailyMovies) {
                    ...
                    if (!boxOfficeMovie.isValidMovie()) {
                        continue;
                    }
                    ...
                    MovieAPI detailMovie = KobisMovieInfoAPI.getMovieDetail(movieId);

                    // 영화 정보 중에 null 값을 가지면 continue
                    ...
                    MovieAPI mergedMovie = mergeMovie(boxOfficeMovie, detailMovie);
     ...
     */
    private static MovieAPI mergeMovie(MovieAPI boxOfficeMovie, MovieAPI detailMovie) {
        MovieAPI mergedMovie = new MovieAPI(
